...
impl BorrowGenerator {
    ...
    pub fn transform_function(
        ...
        let (transformed_params, length_params_to_remove) =
            self.transform_parameters_with_array_detection(
                func.parameters(),
                inferences,
                &dataflow_graph,
            );
    ...
    fn transform_parameters_with_array_detection(
        ...
        for (i, param) in params.iter().enumerate() {
            ...
            if let Some(true) = dataflow_graph.is_array_parameter(param.name()) {
                ...
                let slice_type = self.transform_to_slice_type(param.param_type(), param.name(), inferences, dataflow_graph);
    ...
    fn is_parameter_mutated(
        ...
        dataflow_graph.body().iter().any(|stmt| {
            self.statement_mutates_variable(stmt, var_name)
        })
    }
    ...
    fn statement_mutates_variable(&self, stmt: &decy_hir::HirStatement, var_name: &str) -> bool {
        ...
        match stmt {
            HirStatement::ArrayIndexAssignment { array, .. } => {
                ...
                if let decy_hir::HirExpression::Variable(name) = &**array {
                    name == var_name
                } else {
                    false
                }
            ...
            HirStatement::DerefAssignment { target, .. } => {
                ...
                if let decy_hir::HirExpression::Variable(name) = target {
                    name == var_name
                } else {
                    false
                }
            ...
            HirStatement::If { then_block, else_block, .. } => {
                then_block.iter().any(|s| self.statement_mutates_variable(s, var_name))
                    || else_block
                        .as_ref()
                        .map(|stmts| stmts.iter().any(|s| self.statement_mutates_variable(s, var_name)))
                        .unwrap_or(false)
            }
            HirStatement::While { body, .. } | HirStatement::For { body, .. } => {
                body.iter().any(|s| self.statement_mutates_variable(s, var_name))
            }
            HirStatement::Switch { cases, default_case, .. } => {
                cases.iter().any(|case| {
                    case.body.iter().any(|s| self.statement_mutates_variable(s, var_name))
                }) || default_case
                    .as_ref()
                    .map(|stmts| stmts.iter().any(|s| self.statement_mutates_variable(s, var_name)))
                    .unwrap_or(false)
    ...
    fn transform_statement_with_length_replacement(
        ...
        match stmt {
            HirStatement::Return(expr_opt) => HirStatement::Return(
                expr_opt
                    .as_ref()
                    .map(|e| self.transform_expression_with_length_replacement(e, inferences, length_params_to_remove)),
            ),
            HirStatement::Assignment { target, value } => HirStatement::Assignment {
                target: target.clone(),
                value: self.transform_expression_with_length_replacement(value, inferences, length_params_to_remove),
            },
            HirStatement::DerefAssignment { target, value } => HirStatement::DerefAssignment {
                target: self.transform_expression_with_length_replacement(target, inferences, length_params_to_remove),
                value: self.transform_expression_with_length_replacement(value, inferences, length_params_to_remove),
            },
            HirStatement::ArrayIndexAssignment { array, index, value } => {
                HirStatement::ArrayIndexAssignment {
                    array: Box::new(self.transform_expression_with_length_replacement(array, inferences, length_params_to_remove)),
                    index: Box::new(self.transform_expression_with_length_replacement(index, inferences, length_params_to_remove)),
                    value: self.transform_expression_with_length_replacement(value, inferences, length_params_to_remove),
                }
            }
            HirStatement::FieldAssignment { object, field, value } => {
                HirStatement::FieldAssignment {
                    object: self.transform_expression_with_length_replacement(object, inferences, length_params_to_remove),
                    field: field.clone(),
                    value: self.transform_expression_with_length_replacement(value, inferences, length_params_to_remove),
                }
            }
            HirStatement::If { condition, then_block, else_block } => {
                HirStatement::If {
                    condition: self.transform_expression_with_length_replacement(condition, inferences, length_params_to_remove),
                    then_block: then_block.iter().map(|s| self.transform_statement_with_length_replacement(s, inferences, length_params_to_remove)).collect(),
                    else_block: else_block.as_ref().map(|stmts| {
                        stmts.iter().map(|s| self.transform_statement_with_length_replacement(s, inferences, length_params_to_remove)).collect()
                    }),
                }
            }
            HirStatement::While { condition, body } => HirStatement::While {
                condition: self.transform_expression_with_length_replacement(condition, inferences, length_params_to_remove),
                body: body.iter().map(|s| self.transform_statement_with_length_replacement(s, inferences, length_params_to_remove)).collect(),
            },
            HirStatement::For { init, condition, increment, body } => HirStatement::For {
                init: init.as_ref().map(|s| Box::new(self.transform_statement_with_length_replacement(s, inferences, length_params_to_remove))),
                condition: self.transform_expression_with_length_replacement(condition, inferences, length_params_to_remove),
                increment: increment.as_ref().map(|s| Box::new(self.transform_statement_with_length_replacement(s, inferences, length_params_to_remove))),
                body: body.iter().map(|s| self.transform_statement_with_length_replacement(s, inferences, length_params_to_remove)).collect(),
            },
            HirStatement::Switch { condition, cases, default_case } => HirStatement::Switch {
                ...
                cases: cases
                    ...
                    .map(|case| decy_hir::SwitchCase {
                        ...
                        body: case.body.iter().map(|s| self.transform_statement_with_length_replacement(s, inferences, length_params_to_remove)).collect(),
                    })
                    .collect(),
            ...
            HirStatement::Free { pointer } => HirStatement::Free {
                pointer: self.transform_expression_with_length_replacement(pointer, inferences, length_params_to_remove),
            },
            HirStatement::Expression(expr) => {
                HirStatement::Expression(self.transform_expression_with_length_replacement(expr, inferences, length_params_to_remove))
            }
            HirStatement::VariableDeclaration { name, var_type, initializer } => {
                HirStatement::VariableDeclaration {
                    name: name.clone(),
                    var_type: var_type.clone(),
                    initializer: initializer.as_ref().map(|e| self.transform_expression_with_length_replacement(e, inferences, length_params_to_remove)),
                }
            }
    ...
    fn transform_expression_recursive_with_length(
                        ...
                        if let Some(inference) = inferences.get(var_name) {
                            if let crate::inference::OwnershipKind::ArrayPointer { element_type, .. } = &inference.kind {
                                let index_expr = if matches!(op, BinaryOperator::Subtract) {
                                    self.transform_expression_with_length_replacement(right, inferences, length_params_to_remove)
                                } else {
                                    self.transform_expression_with_length_replacement(right, inferences, length_params_to_remove)
                                };
        ...
        match expr {
            HirExpression::Dereference(inner) => {
                HirExpression::Dereference(Box::new(self.transform_expression_with_length_replacement(inner, inferences, length_params_to_remove)))
            }
            HirExpression::AddressOf(inner) => {
                HirExpression::AddressOf(Box::new(self.transform_expression_with_length_replacement(inner, inferences, length_params_to_remove)))
            }
            HirExpression::UnaryOp { op, operand } => HirExpression::UnaryOp {
                op: *op,
                operand: Box::new(self.transform_expression_with_length_replacement(operand, inferences, length_params_to_remove)),
            },
            HirExpression::BinaryOp { op, left, right } => HirExpression::BinaryOp {
                op: *op,
                left: Box::new(self.transform_expression_with_length_replacement(left, inferences, length_params_to_remove)),
                right: Box::new(self.transform_expression_with_length_replacement(right, inferences, length_params_to_remove)),
            },
            HirExpression::FunctionCall { function, arguments } => HirExpression::FunctionCall {
                function: function.clone(),
                arguments: arguments
                    .iter()
                    .map(|arg| self.transform_expression_with_length_replacement(arg, inferences, length_params_to_remove))
                    .collect(),
            },
            HirExpression::FieldAccess { object, field } => HirExpression::FieldAccess {
                object: Box::new(self.transform_expression_with_length_replacement(object, inferences, length_params_to_remove)),
                field: field.clone(),
            },
            HirExpression::PointerFieldAccess { pointer, field } => HirExpression::PointerFieldAccess {
                pointer: Box::new(self.transform_expression_with_length_replacement(pointer, inferences, length_params_to_remove)),
                field: field.clone(),
            },
            ...
            HirExpression::Cast { expr: cast_expr, target_type } => HirExpression::Cast {
                expr: Box::new(self.transform_expression_with_length_replacement(cast_expr, inferences, length_params_to_remove)),
            ...
            HirExpression::CompoundLiteral { literal_type, initializers } => HirExpression::CompoundLiteral {
                ...
                initializers: initializers
                    .iter()
                    .map(|init| self.transform_expression_with_length_replacement(init, inferences, length_params_to_remove))
                    .collect(),
            },
            HirExpression::IsNotNull(inner) => {
                HirExpression::IsNotNull(Box::new(self.transform_expression_with_length_replacement(inner, inferences, length_params_to_remove)))
            }
            HirExpression::Calloc { count, element_type } => HirExpression::Calloc {
                count: Box::new(self.transform_expression_with_length_replacement(count, inferences, length_params_to_remove)),
                element_type: element_type.clone(),
            },
            HirExpression::Malloc { size } => HirExpression::Malloc {
                size: Box::new(self.transform_expression_with_length_replacement(size, inferences, length_params_to_remove)),
            },
            HirExpression::Realloc { pointer, new_size } => HirExpression::Realloc {
                pointer: Box::new(self.transform_expression_with_length_replacement(pointer, inferences, length_params_to_remove)),
                new_size: Box::new(self.transform_expression_with_length_replacement(new_size, inferences, length_params_to_remove)),
            },
            HirExpression::StringMethodCall { receiver, method, arguments } => {
                HirExpression::StringMethodCall {
                    receiver: Box::new(self.transform_expression_with_length_replacement(receiver, inferences, length_params_to_remove)),
                    method: method.clone(),
                    arguments: arguments
                        .iter()
                        .map(|arg| self.transform_expression_with_length_replacement(arg, inferences, length_params_to_remove))
            ...
            HirExpression::SliceIndex { slice, index, element_type } => {
                HirExpression::SliceIndex {
                    slice: Box::new(self.transform_expression_with_length_replacement(slice, inferences, length_params_to_remove)),
                    index: Box::new(self.transform_expression_with_length_replacement(index, inferences, length_params_to_remove)),
                    element_type: element_type.clone(),
                }
            }
