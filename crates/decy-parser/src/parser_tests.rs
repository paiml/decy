...
mod tests {
    ...
    fn test_parse_simple_main_function() {
        ...
        let ast = parser.parse(source).expect("Parsing simple main function should succeed");

        // Verify we got one function
        assert_eq!(ast.functions().len(), 1, "Should parse exactly one function");
    ...
    fn test_parse_function_with_parameters() {
        ...
        let ast = parser.parse(source).expect("Parsing function with parameters should succeed");
    ...
    fn test_parse_function_with_return_value() {
        ...
        let ast = parser.parse(source).expect("Parsing function with return value should succeed");
    ...
    fn test_parse_syntax_error_handling() {
        ...
        assert!(result.is_err(), "Parsing invalid syntax should return an error");
    }
    ...
    fn test_parse_empty_input() {
        ...
        if let Ok(ast) = result {
            assert_eq!(ast.functions().len(), 0, "Empty input should have no functions");
        }
    }
    ...
    fn test_parse_multiple_functions() {
        ...
        let ast = parser.parse(source).expect("Parsing multiple functions should succeed");
    ...
    fn test_parse_void_function() {
        ...
        let ast = parser.parse(source).expect("Parsing void function should succeed");
    ...
    fn test_parse_pointer_parameter() {
        ...
        let ast = parser.parse(source).expect("Parsing pointer parameter should succeed");
    ...
    fn test_parse_double_type() {
        ...
        let ast = parser.parse(source).expect("Parsing double function should succeed");

        let func = &ast.functions()[0];
        assert_eq!(func.name, "calculate_pi");
        assert_eq!(func.return_type, Type::Double, "Return type should be double");
    }
    ...
    fn test_parse_double_parameter() {
        ...
        let ast =
            parser.parse(source).expect("Parsing function with double parameter should succeed");

        let func = &ast.functions()[0];
        assert_eq!(func.parameters.len(), 1);
        assert_eq!(func.parameters[0].param_type, Type::Double, "Parameter should be double");
    }
    ...
    fn test_parse_char_type() {
        ...
        let ast = parser.parse(source).expect("Parsing char function should succeed");
    ...
    fn test_parse_char_parameter() {
        ...
        let ast =
            parser.parse(source).expect("Parsing function with char parameter should succeed");

        let func = &ast.functions()[0];
        assert_eq!(func.parameters.len(), 1);
        assert_eq!(func.parameters[0].param_type, Type::Char, "Parameter should be char");
    }
    ...
    fn test_parse_mixed_types() {
        ...
        let ast = parser.parse(source).expect("Parsing function with mixed types should succeed");
    ...
    fn test_parse_return_literal_value() {
        ...
        let ast = parser.parse(source).expect("Parsing return with literal should succeed");
    ...
    fn test_parse_binary_expression() {
        ...
        let ast = parser.parse(source).expect("Parsing binary expression should succeed");
    ...
    fn test_parse_assignment_statement() {
        ...
        let ast = parser.parse(source).expect("Parsing assignment should succeed");

        let func = &ast.functions()[0];
        assert_eq!(func.name, "set_value");
        assert_eq!(func.body.len(), 2, "Should have two statements: declaration and assignment");
    ...
    fn test_parse_if_statement() {
        ...
        let ast = parser.parse(source).expect("Parsing if statement should succeed");
    ...
    fn test_parse_if_else_statement() {
        ...
        let ast = parser.parse(source).expect("Parsing if-else statement should succeed");
        ...
        match &func.body[0] {
            Statement::If { condition, then_block, else_block } => {
                ...
                assert!(matches!(condition, Expression::BinaryOp { .. }), "Should have comparison");
    ...
    fn test_parse_for_loop() {
        ...
        let ast = parser.parse(source).expect("Parsing for loop should succeed");
    ...
    fn test_parse_nested_if() {
        ...
        let ast = parser.parse(source).expect("Parsing nested if should succeed");

        let func = &ast.functions()[0];
        assert_eq!(func.name, "classify");
        // Should have: if (outer), return
        assert!(func.body.len() >= 2, "Should have at least outer if and final return");

        // Outer if statement
        match &func.body[0] {
            Statement::If { then_block, else_block, .. } => {
                // Then block should have nested if
                assert!(then_block.len() >= 2, "Should have nested if and return in then block");
                match &then_block[0] {
                    Statement::If { .. } => {
                        // Nested if found
                    }
    ...
    fn test_parse_while_loop_with_body() {
        ...
        let ast = parser.parse(source).expect("Parsing while loop should succeed");
    ...
    fn test_parse_pointer_dereference_in_return() {
        ...
        let ast = parser.parse(source).expect("Parsing pointer dereference should succeed");
    ...
    fn test_parse_pointer_dereference_in_condition() {
        ...
        let ast = parser.parse(source).expect("Parsing dereference in condition should succeed");
    ...
    fn test_parse_pointer_dereference_in_assignment() {
        ...
        let ast = parser.parse(source).expect("Parsing dereference in assignment should succeed");

        let func = &ast.functions()[0];
        assert_eq!(func.name, "copy");
        assert_eq!(func.body.len(), 3, "Should have var decl and two assignments");
    ...
    fn test_parse_logical_and() {
        ...
        let ast = parser.parse(source).expect("Parsing logical AND should succeed");
            ...
            Statement::If { condition, .. } => {
                match condition {
                    Expression::BinaryOp { op, left, right } => {
                        ...
                        // Left: a > 0
                        match **left {
                            Expression::BinaryOp { op: ref left_op, .. } => {
                        ...
                        // Right: b > 0
                        match **right {
                            Expression::BinaryOp { op: ref right_op, .. } => {
    ...
    fn test_parse_logical_or() {
        ...
        let ast = parser.parse(source).expect("Parsing logical OR should succeed");
    ...
    fn test_parse_combined_logical_ops() {
        ...
        let ast = parser.parse(source).expect("Parsing combined logical ops should succeed");
    ...
    fn test_parse_array_index_in_expression() {
        ...
        let ast = parser.parse(source).expect("Parsing array indexing should succeed");
    ...
    fn test_parse_array_index_assignment() {
        ...
        let ast = parser.parse(source).expect("Parsing array index assignment should succeed");
    ...
    fn test_parse_array_index_in_binary_expr() {
        ...
        let ast =
            parser.parse(source).expect("Parsing array index in binary expression should succeed");
    ...
    fn test_parse_pointer_field_access() {
        ...
        let ast = parser.parse(source).expect("Parsing pointer field access should succeed");

        // Second function (index 1) because struct declaration is parsed too
        assert!(!ast.functions().is_empty(), "Should have at least one function");
        let func = &ast.functions()[0];
    ...
    fn test_parse_field_access() {
        ...
        let ast = parser.parse(source).expect("Parsing field access should succeed");
    ...
    fn test_parse_pointer_field_access_assignment() {
        ...
        let ast =
            parser.parse(source).expect("Parsing pointer field access assignment should succeed");
    ...
    fn test_parse_unary_minus() {
        ...
        let ast = parser.parse(source).expect("Parsing unary minus should succeed");
    ...
    fn test_parse_logical_not() {
        ...
        let ast = parser.parse(source).expect("Parsing logical NOT should succeed");
    ...
    fn test_parse_double_negative() {
        ...
        let ast = parser.parse(source).expect("Parsing double negative should succeed");
        ...
        match &func.body[0] {
            Statement::Return(Some(Expression::UnaryOp {
                ...
                assert_eq!(*outer_op, UnaryOperator::Minus, "Outer operator should be minus");

                // Inner should also be UnaryOp with minus
                match **outer_operand {
                    Expression::UnaryOp { ref op, ref operand } => {
    ...
    fn test_parse_function_call_with_variable_args() {
        ...
        let ast =
            parser.parse(source).expect("Parsing function call with variable args should succeed");
        ...
        match &compute_func.body[0] {
            Statement::VariableDeclaration { name, initializer, .. } => {
                ...
                match initializer {
                    Some(Expression::FunctionCall { function, arguments }) => {
    ...
    fn test_parse_nested_function_calls() {
        ...
        let ast = parser.parse(source).expect("Parsing nested function calls should succeed");
        ...
        match &nested_func.body[0] {
            Statement::Return(Some(Expression::FunctionCall { function, arguments })) => {
                assert_eq!(function, "add");
    ...
    fn test_parse_struct_value_parameter() {
        ...
        let ast = parser.parse(source).expect("Parsing struct value parameter should succeed");
        ...
        match &func.parameters[0].param_type {
            ...
            _ => {
                panic!("Parameter should be a struct type, got {:?}", func.parameters[0].param_type)
            }
        }
    }
    ...
    fn test_parse_struct_pointer_parameter() {
        ...
        let ast = parser.parse(source).expect("Parsing struct pointer parameter should succeed");
    ...
    fn test_parse_struct_return_type() {
        ...
        let ast = parser.parse(source).expect("Parsing struct return type should succeed");
    ...
    fn test_parse_simple_typedef() {
        ...
        let ast = parser.parse(source).expect("Parsing simple typedef should succeed");

        // Should have one typedef and one function
        assert!(!ast.typedefs().is_empty(), "Should have at least one typedef");
        let typedef = &ast.typedefs()[0];
        assert_eq!(typedef.name, "Integer", "Typedef name should be 'Integer'");
        assert_eq!(typedef.underlying_type, Type::Int, "Underlying type should be int");
    ...
    fn test_parse_struct_typedef() {
        ...
        let ast = parser.parse(source).expect("Parsing struct typedef should succeed");
    ...
    fn test_parse_pointer_typedef() {
        ...
        let ast = parser.parse(source).expect("Parsing pointer typedef should succeed");
    ...
    fn test_parse_function_pointer_typedef() {
        ...
        let ast = parser.parse(source).expect("Parsing function pointer typedef should succeed");

        // Should have one typedef
        assert!(!ast.typedefs().is_empty(), "Should have at least one typedef");
        let typedef = &ast.typedefs()[0];
        assert_eq!(typedef.name, "Callback", "Typedef name should be 'Callback'");
        ...
        match &typedef.underlying_type {
            ...
            _ => {
                panic!("Typedef should be function pointer type, got {:?}", typedef.underlying_type)
            }
        }
    }
    ...
    fn test_parse_function_pointer_parameter() {
        ...
        let ast = parser.parse(source).expect("Parsing function pointer parameter should succeed");

        assert!(!ast.functions().is_empty(), "Should have at least one function");
        let func = &ast.functions()[0];
        assert_eq!(func.name, "register_callback");
        assert_eq!(func.parameters.len(), 1, "Should have one parameter");
        assert_eq!(func.parameters[0].name, "cb", "Parameter name should be 'cb'");
    ...
    fn test_parse_function_pointer_return_type() {
        ...
        let ast =
            parser.parse(source).expect("Parsing function pointer return type should succeed");
        ...
        match &func.return_type {
            ...
            _ => panic!("Return type should be function pointer, got {:?}", func.return_type),
        }
    }
    ...
    fn test_parse_string_literal_in_return() {
        ...
        let ast = parser.parse(source).expect("Parsing string literal should succeed");
    ...
    fn test_parse_string_literal_in_variable() {
        ...
        let ast = parser.parse(source).expect("Parsing string in variable should succeed");
    ...
    fn test_parse_string_literal_in_function_call() {
        ...
        let ast = parser.parse(source).expect("Parsing string in function call should succeed");
    ...
    fn test_visitor_unknown_statement_cursor() {
        ...
        let ast =
            parser.parse(source).expect("Parsing with unsupported statement should still succeed");
    ...
    fn test_visitor_unknown_expression_cursor() {
        ...
        let ast = parser.parse(source).expect("Parsing with complex cast should succeed");
    ...
    fn test_visitor_unknown_call_argument() {
        ...
        let ast =
            parser.parse(source).expect("Parsing function call with complex args should succeed");
    ...
    fn test_extract_var_decl_returns_none() {
        ...
        let ast = parser.parse(source).expect("Parsing simple var decl should succeed");
    ...
    fn test_extract_return_stmt_with_no_value() {
        ...
        let ast = parser.parse(source).expect("Parsing return with no value should succeed");
    ...
    fn test_extract_binary_op_with_no_operator() {
        ...
        let ast = parser.parse(source).expect("Parsing binary operation should succeed");
    ...
    fn test_extract_binary_operator_division() {
        ...
        let ast = parser.parse(source).expect("Parsing division should succeed");
    ...
    fn test_extract_binary_operator_subtract() {
        ...
        let ast = parser.parse(source).expect("Parsing subtraction should succeed");

        let func = &ast.functions()[0];
        match &func.body[0] {
            Statement::Return(Some(Expression::BinaryOp { op, .. })) => {
                assert_eq!(*op, BinaryOperator::Subtract, "Should be subtraction operator");
            }
    ...
    fn test_extract_int_literal_null_tu() {
        ...
        let ast = parser.parse(source).expect("Parsing integer literal should succeed");
    ...
    fn test_extract_string_literal_empty() {
        ...
        let ast = parser.parse(source).expect("Parsing empty string should succeed");
    ...
    fn test_extract_function_call_no_args() {
        ...
        let ast = parser.parse(source).expect("Parsing function call with no args should succeed");
    ...
    fn test_convert_type_elaborated() {
        ...
        let ast = parser.parse(source).expect("Parsing elaborated type should succeed");
    ...
    fn test_convert_type_typedef_canonical() {
        ...
        let ast = parser.parse(source).expect("Parsing typedef should succeed");

        let func = &ast.functions()[0];
        // Clang resolves typedefs to canonical types
        assert_eq!(func.return_type, Type::Int, "Should resolve typedef to int");
        assert_eq!(func.parameters[0].param_type, Type::Int, "Should resolve typedef to int");
    }
    ...
    fn test_assignment_with_complex_rhs() {
        ...
        let ast = parser.parse(source).expect("Parsing assignment with complex RHS should succeed");
    ...
    fn test_assignment_distinguishes_equality() {
        ...
        let ast = parser.parse(source).expect("Parsing == comparison should succeed");
    ...
    fn test_deref_assignment_edge_case() {
        ...
        let ast =
            parser.parse(source).expect("Parsing nested dereference assignment should succeed");
    ...
    fn test_field_assignment_with_struct_value() {
        ...
        let ast = parser.parse(source).expect("Parsing field assignment should succeed");
    ...
    fn test_assignment_chain() {
        ...
        let ast = parser.parse(source).expect("Parsing assignment chain should succeed");

        let func = &ast.functions()[0];
        assert_eq!(func.body.len(), 5, "Should have 2 var decls + 3 assignments");
    ...
    fn test_if_with_unary_in_condition() {
        ...
        let ast = parser.parse(source).expect("Parsing if with unary in condition should succeed");
    ...
    fn test_if_with_integer_literal_condition() {
        ...
        let ast = parser.parse(source).expect("Parsing if with integer literal should succeed");
    ...
    fn test_if_with_variable_condition() {
        ...
        let ast = parser.parse(source).expect("Parsing if with variable condition should succeed");
    ...
    fn test_for_loop_with_variable_in_condition() {
        ...
        let ast = parser.parse(source).expect("Parsing for with variable condition should succeed");
    ...
    fn test_while_loop_with_variable_condition() {
        ...
        let ast =
            parser.parse(source).expect("Parsing while with variable condition should succeed");
    ...
    fn test_while_loop_with_function_call_condition() {
        ...
        let ast = parser.parse(source).expect("Parsing while with call condition should succeed");
    ...
    fn test_function_call_with_array_index_argument() {
        ...
        let ast = parser.parse(source).expect("Parsing call with array arg should succeed");
    ...
    fn test_function_call_with_unary_argument() {
        ...
        let ast = parser.parse(source).expect("Parsing call with unary arg should succeed");
    ...
    fn test_function_call_with_field_access_argument() {
        ...
        let ast = parser.parse(source).expect("Parsing call with field access arg should succeed");
    ...
    fn test_while_with_integer_literal_condition() {
        ...
        let ast = parser.parse(source).expect("Parsing while with int literal should succeed");
    ...
    fn test_while_with_unary_in_condition() {
        ...
        let ast = parser.parse(source).expect("Parsing while with unary should succeed");
    ...
    fn test_for_with_integer_literal_condition() {
        ...
        let ast =
            parser.parse(source).expect("Parsing for with int literal condition should succeed");
    ...
    fn test_for_with_function_call_condition() {
        ...
        let ast = parser.parse(source).expect("Parsing for with call condition should succeed");
    ...
    fn test_if_with_function_call_condition() {
        ...
        let ast = parser.parse(source).expect("Parsing if with call condition should succeed");
    ...
    fn test_parse_sizeof_struct() {
        ...
        let ast = parser.parse(source).expect("Parsing sizeof struct should succeed");
